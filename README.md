...
1. **人脸识别**：拖拽上传 Ref_Figure 人脸库 / Target_Figure 出工照片 → 点「开始人脸识别」→ 生成 `output/00_程序人脸识别结果.xlsx`（后台线程运行，页面实时刷新日志）。识别完成后若存在未识别(Unknown)人脸，会自动弹出**人工复核**弹窗：逐张展示照片并红色框选未识别人脸，旁边显示照片人员名单（自动匹配 `input/01_照片台账表.xlsx` 中对应照片的人员名单）/ 拍照人（不在照片中，不算异常）/ 识别结果 / 差异提示，每个未识别框可填姓名（可留空跳过）→ 点「确认本张」自动写回 `00_`，后续表3~表10 直接使用修正后的姓名
2. **上传输入表**：上传 5 个输入表（照片台账表 / 工作安排表 / 人员分类表 / 项目信息表 / 考勤模板），全部就绪后进入下一步
3. **生成表格**：一键依次生成表1~表10，生成到关键表时自动弹出查看/编辑界面——**表1** 弹出预览并红色高亮开工/收工汇报异常；**05/06** 弹出核对界面（内联展示核对信息错误文档 + 可直接在网页上修改表格：改单元格 / 增行 / 删行；05 的「加班时长确认-人工审核」列空值显示「待确认」供填写确认，06 该列锁定不可改，可修改的是开工/收工项目名与简称），点「保存并继续」写回 output 对应表格后继续
4. **完成下载**：全部完成后下载「08_09_10_最终考勤表.xlsx」（**一个文件三个子表**：表8工程考勤表 / 表9外协考勤表1 / 表10外协考勤表2，合并单元格格式保留，与输入模板同构）
...
| `web_show/web_app.py` | Flask Web 交互界面（`AI_PY312` 环境启动，浏览器打开 `http://localhost:5000`）。苹果风格白蓝主题分步向导：①上传照片→人脸识别（含人工复核弹窗）②上传输入表 ③生成表格（表1 预览高亮异常；05/06 网页内联展示错误文档+可编辑表格并写回）④完成下载（合并文件「08_09_10_最终考勤表.xlsx」一个文件三个子表）。识别在后台线程运行、前端实时轮询日志 |
